...
class OverlaySelector<T> @JvmOverloads constructor(label: String, var selected: T, val callback: Consumer<T>, val items: Supplier<Array<T>>, var enabled: Boolean = true) : OverlayLabel(label,enabled, Runnable {  }) {
    ...
    override fun render(mouseX: Int, mouseY: Int, overlayX: Int, overlayY: Int, w: Int, h: Int, overlayH: Int): Boolean {
        ...
        val textY = overlayY + (h - OldHyperiumMainGui.INSTANCE.fr.FONT_HEIGHT) / 2
        if(super.enabled) {
            OldHyperiumMainGui.INSTANCE.fr.drawString(selected.toString(), overlayX + w - OldHyperiumMainGui.INSTANCE.fr.getWidth(selected.toString()) - 5, textY.toFloat(), 0xffffff)
        } else{
            OldHyperiumMainGui.INSTANCE.fr.drawString(selected.toString(), overlayX + w - OldHyperiumMainGui.INSTANCE.fr.getWidth(selected.toString()) - 5, textY.toFloat(), Color(169, 169, 169).rgb)
        }
        return true
    }
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, overlayX: Int, overlayY: Int, w: Int, h: Int) {
        ...
        if (mouseX >= overlayX + w - OldHyperiumMainGui.INSTANCE.fr.getWidth(selected.toString()) - 5 && mouseX <= overlayX + w - 5 && mouseY >= overlayY + 5 && mouseY <= overlayY + h - 5) {
            val tmp = items.get()
