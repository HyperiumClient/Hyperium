    ...
    override fun transform(original: ClassNode): ClassNode {
        ...
        for (method in original.methods) {
            if (method.name == "<init>") {
                ...
                method.instructions.insertBefore(method.instructions.first, createHyperiumCapeHandler)
            }

            if (method.name == "getPlayerInfo") {
                method.access = Opcodes.ACC_PUBLIC
            }

            if (method.name == "getLocationSkin" && method.desc == "()Lnet/minecraft/util/ResourceLocation;") {
                ...
                method.instructions.insertBefore(method.instructions.first, createNickHider)
            }
            ...
            if (method.name == "getFovModifier") {
                ...
                for (insn in method.instructions.iterator()) {
                    if (insn.opcode == Opcodes.FRETURN) {
                        method.instructions.insertBefore(insn, setNewReturn)
                    }
                }
            }
        }
