...
public class GeneralChatHandler {
    ...
    public void sendMessage(String message, boolean addHeader) {
        if (addHeader) {
            message = ChatColor.RED + "[HCC] " + ChatColor.WHITE.toString() + message;
        }
    ...
    public void chatEvent(ChatEvent event) {
        for (HCCChatHandler hccChatHandler : handlerList) {
            ...
            try {
                hccChatHandler.chatReceived(event.getChat(), ChatColor.stripColor(event.getChat().getUnformattedText()));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
