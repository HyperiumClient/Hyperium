...
import com.hcc.installer.components.MotionPanel;
import com.hcc.installer.utils.DownloadTask;
...
public class InstallerFrame extends JFrame implements PropertyChangeListener {
    ...
    InstallerFrame() {
        ...
        setTitle("HCC Installer");

        setIconImage(new ImageIcon(getClass().getResource("/assets/hcc/icons/icon-32x.png")).getImage());
    ...
    private void install() {
        ...
        File target = new File(versions, "HCC 1.8.9");
        if (target.exists()) {
            try {
                delete(target);
            } catch (IOException e) {
        ...
        String versions_url = "https://raw.githubusercontent.com/HypixelCommunityClient/HCC-Repo/master/installer/versions.json";
        ...
        try{
            ...
            File dl = new File(new File(mc, "libraries"), version.get().getString("path"));
        ...
        //noinspection ResultOfMethodCallIgnored
        target.mkdir();
        File targetJson = new File(target, "HCC 1.8.9.json");
        File targetJar = new File(target, "HCC 1.8.9.jar");
        try {
        ...
        error.setText("OPEN LAUNCHER AND LAUNCH 'HCC 1.8.9' PROFILE");
        exit.setVisible(true);
    }
    ...
    private void initComponents() {
        ...
        try {
            display.setFont(Font.createFont(Font.TRUETYPE_FONT, InstallerMain.class.getResourceAsStream("/assets/hcc/fonts/Montserrat-Regular.ttf")).deriveFont(15.0F));
        ...
        try {
            error.setFont(Font.createFont(Font.TRUETYPE_FONT, InstallerMain.class.getResourceAsStream("/assets/hcc/fonts/Montserrat-Regular.ttf")).deriveFont(10.0F));
        } catch (FontFormatException | IOException e) {
        ...
        try {
            exit.setFont(Font.createFont(Font.TRUETYPE_FONT, InstallerMain.class.getResourceAsStream("/assets/hcc/fonts/Montserrat-Regular.ttf")).deriveFont(12.0F));
        } catch (FontFormatException | IOException e) {
