  ...
  public void print() {
    ...
    gameStatistics.entrySet().stream()
        ...
        .forEach(entry -> {
          ...
          messageUtil.getKeyHelper("command.stats")
              .withKey("coins", context -> context.getBuilder(game, coins.getTotal())
                  .setHover(context.getKey("coinsHover"), game, coins.getSent(),
                      coins.getReceived())
                  .send());
        });
    messageUtil.getKeyHelper("command.stats")
        .withKey("tips", context -> context.getBuilder(getTipsTotal())
