...
public class CommandDebug implements BaseCommand {
  ...
  private static void tryChromaHUD(StringBuilder builder) {
    try {
      builder.append("ChromaHUD: ")
          .append(printer.toJson(ChromaHUDApi.getInstance().getConfig().getObject()));
    } catch (Exception e) {
  ...
  private static void tryLevelhead(StringBuilder builder) {
    try {
      builder.append("Count: ").append(Hyperium.INSTANCE.getModIntegration().getLevelhead().count)
          .append("\n");
      builder.append("Wait: ").append(Hyperium.INSTANCE.getModIntegration().getLevelhead().wait)
          .append("\n");
      ...
      builder.append("Local Stats: ").append(HypixelDetector.getInstance().isHypixel())
          .append("\n");
  ...
  public static String get() {
    ...
    if (isPremium) {
      builder.append("Premium: True, ").append("UUID is ")
          .append(Minecraft.getMinecraft().thePlayer.getGameProfile().getId());
    } else {
  ...
  public void onExecute(String[] args) {
    if (args.length == 1 && args[0].equalsIgnoreCase("log")) {
      ...
      GeneralChatHandler.instance().sendMessage(
          "Enable logging, please restart your game to begin. It will be auto disabled after the next launch");
    }
  }
