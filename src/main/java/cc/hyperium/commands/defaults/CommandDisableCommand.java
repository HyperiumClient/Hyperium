...
public class CommandDisableCommand implements BaseCommand {
  ...
  private HyperiumCommandHandler handler = Hyperium.INSTANCE.getHandlers()
      .getHyperiumCommandHandler();
  ...
  public void onExecute(String[] args) {
    if (args.length == 0) {
      Hyperium.INSTANCE.getHandlers().getGeneralChatHandler()
          .sendMessage("Please input the command to disable");
      return;
    }
    ...
    if (command.equalsIgnoreCase("disablecommand") || command.equalsIgnoreCase("hyperium")) {
      Hyperium.INSTANCE.getHandlers().getGeneralChatHandler()
          .sendMessage("That command cannot be disabled!");
      return;
    }
    ...
    Hyperium.INSTANCE.getHandlers().getGeneralChatHandler()
        .sendMessage("Command '" + args[0] + "' is " + color +
