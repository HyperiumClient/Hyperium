...
/**
 * Our implementation of the normal Minecraft KeyBinding, uses anonymous classes instead of using an
 * onTick event, this is debatably better
 *
 * @author CoalOres
...
public class HyperiumBind {
  ...
  /**
   * Setter for the key code, sets it here and calls the super method to update it as well
   *
   * @param key the key
   */
  ...
  /**
   * Returns the key description, this will be capitalized if the capitalize method returns true
   * (see {@link #capitalizeDescription()})
   *
  ...
  /**
   * Setter for the WasPressed variable, if true it means the buttons last event was to be pressed
   *
  ...
  /**
   * Setter for the conflicted variable, if true it means the bind is conflicting with another.
   */
  ...
  /**
   * Should the Key Description be capitalized, this looks neater in the controls menu, instead of
  ...
  public void detectConflicts() {
    ...
    List<HyperiumBind> otherBinds = new ArrayList<>(
        Hyperium.INSTANCE.getHandlers().getKeybindHandler().getKeybinds().values());
  ...
  public KeyBinding toKeyBind() {
    return new KeyBinding(description, getKeyCode(),
        "key.categories.hyperium" + keyType.keyCategory, defaultKeyCode);
  }
