...
public class ArmWaveKeybind extends HyperiumBind {
    ...
    public void onPress() {
        ...
        if (Settings.ARM_WAVE_TOGGLE_MODE) {
            ...
            if (armWaveToggled) armWaveHandler.startAnimation(uuid);
            else armWaveHandler.stopAnimation(uuid);

            if (client != null) {
                client.write(ServerCrossDataPacket.build(new JsonHolder().put("type", "armwave_update").put("posing", armWaveToggled)));
            }
    ...
    public void onRelease() {
        if (!Settings.ARM_WAVE_TOGGLE_MODE) armWaveToggled = false;
        if (Settings.ARM_WAVE_TOGGLE || Settings.ARM_WAVE_TOGGLE_MODE) return;
