...
public class ArmWaveHandler extends AbstractPreCopyAnglesAnimationHandler {
  ...
  public void onRender() {
    if (!get(Minecraft.getMinecraft().thePlayer.getUniqueID()).isAnimating() && get(
        Minecraft.getMinecraft().thePlayer.getUniqueID()).shouldReset()) {
      state = 0.0f;
    }
  }
  ...
  public float modifyState() {
    ...
    while (systemTime < Minecraft.getSystemTime() + (1000 / 120)) {
      state = HyperiumGui
          .clamp(HyperiumGui.easeOut(state, asc ? 100.0f : 0.0f, 0.01f, 7), 0.0f, 100.0f);
