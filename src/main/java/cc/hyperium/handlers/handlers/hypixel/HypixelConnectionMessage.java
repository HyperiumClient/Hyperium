...
public class HypixelConnectionMessage {
  ...
  public void playerJoin(PlayerJoinHypixelEvent event) {
    if (event.getUsername() != null && Settings.CUSTOM_JOIN_LEAVE_MESSAGES
      ...
      Multithreading.runAsync(() -> {
        try {
          String myGuild = HypixelAPI.INSTANCE
              .getGuildFromPlayer(Minecraft.getMinecraft().thePlayer.getName()).get().getName();
          String theirGuild = HypixelAPI.INSTANCE.getGuildFromPlayer(event.getUsername()).get()
              .getName();
          ...
          Hyperium.INSTANCE.getHandlers().getNotificationCenter().display(event.getUsername(),
              ...
              () -> Minecraft.getMinecraft().thePlayer
                  .sendChatMessage("/msg " + event.getUsername() + " Hey!"),
  ...
  public void chat(ChatEvent event) {
    if (Settings.CUSTOM_JOIN_LEAVE_MESSAGES && Hyperium.INSTANCE.getHandlers().getHypixelDetector()
        .isHypixel()) {

      Pattern hypixelLeaveMessage = Pattern.compile("^(?<player>\\S{1,16}) left\\.$");
      Matcher leaveMessageMatcher = hypixelLeaveMessage
          .matcher(event.getChat().getUnformattedText());
