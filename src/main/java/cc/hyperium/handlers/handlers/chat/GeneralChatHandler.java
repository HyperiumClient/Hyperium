...
public class GeneralChatHandler {
  ...
  /**
   * Queues a {@link IChatComponent} to send to the client. If the component is null, an empty
   * message will be queued instead.
   *
  ...
  public void sendMessage(String message, boolean addHeader) {
    ...
    if (addHeader) {
      message =
          Settings.HYPERIUM_CHAT_PREFIX ? ChatColor.RED + "[Hyperium] " + ChatColor.WHITE + message
  ...
  /**
   * Queues a message to send to the client, this message will contain a Hyperium prefix. If the
   * message is null, nothing will be queued.
   *
  ...
  public void post() {
    JsonHolder data = new JsonHolder(
        new JsonParser().parse(new InputStreamReader(GeneralChatHandler.class
    ...
    for (HyperiumChatHandler.ChatRegexType type : HyperiumChatHandler.ChatRegexType.values()) {
      if (!data.has(type.name().toLowerCase())) {
        Hyperium.LOGGER.error(
            "Could not find chat regex type " + type.name().toLowerCase() + " in the remote file.");
        continue;
      }
      HyperiumChatHandler.regexPatterns
          .put(type, Pattern.compile(data.optString(type.name().toLowerCase())));
    }
