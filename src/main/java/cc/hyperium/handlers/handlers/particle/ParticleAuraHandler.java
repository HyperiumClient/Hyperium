...
public class ParticleAuraHandler {
  ...
  private EnumMap<EnumParticleType, IParticle> renderEngines = new EnumMap<>(
      EnumParticleType.class);
  ...
  public void loadPurchaseEvent(PurchaseLoadEvent purchaseLoadEvent) {
    ...
    ParticleAura max_age = new ParticleAura(renderEngines.get(type), particle_animation,
        data.optInt("max_age", 2), chroma, rgb);
  ...
  public void renderPlayer(RenderPlayerEvent event) {
    ...
    if (!entity.equals(Minecraft.getMinecraft().thePlayer) && (entity.posX != entity.prevPosZ
        || entity.posY != entity.prevPosY || entity.posZ != entity.prevPosZ)) {
      return;
    }
      ...
      for (Vec3 vec3 : render) {
        ...
        if (type != null) {
          ...
          if (particleAura.isChroma()) {
            ...
            entityFX.setRBGColorF(color.getRed() / 255F, color.getGreen() / 255F,
                color.getBlue() / 255F);
          } else if (particleAura.isRgb()) {
            entityFX.setRBGColorF(particleAura.getRed() / 255F, particleAura.getBlue() / 255F,
                particleAura.getBlue() / 255F);
          }
