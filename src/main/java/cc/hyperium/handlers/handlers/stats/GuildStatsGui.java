...
public class GuildStatsGui extends HyperiumGui {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    drawScaledText(guild.getName() + " " + guild.getFormatedTag(), current.getScaledWidth() / 2, 30,
        3,
        -1, true, true);
    ...
    fontRendererObj.drawString(
        boldText("Founded: ", HypixelApiPlayer.DMY.format(new Date(guild.getCreateDate()))),
        columnX,
        100 - offsetLeftColumn, -1, true);
    fontRendererObj
        .drawString(boldText("Members: ", WebsiteUtils.comma(guild.getMembers().size())), columnX,
            110 - offsetLeftColumn, -1, true);
    fontRendererObj
        .drawString(boldText("Guild Coins: ", WebsiteUtils.comma(guild.getTotalGuildCoins())),
            columnX,
            120 - offsetLeftColumn, -1, true);
    fontRendererObj
        .drawString(boldText("Legacy Ranking: ", WebsiteUtils.comma(guild.getLegacyRanking())),
            columnX,
            130 - offsetLeftColumn, -1, true);
    List<String> strings = fontRendererObj
        .listFormattedStringToWidth(boldText("Description: ", guild.getDescription()),
            current.getScaledWidth() / 4);
    int i = 0;
    ...
    fontRendererObj
        .drawString(boldText("Guild Level: ", Double.toString(guild.getLevel())), columnX,
            150 + i - offsetLeftColumn, -1, true);
    fontRendererObj
        .drawString(boldText("Total Wins: ", Integer.toString(guild.getWins("total"))), columnX,
    ...
    for (GameType gameType : GameType.values()) {
      ...
      fontRendererObj.drawString(boldText(gameType.getName() + " Wins: ",
          Integer.toString(guild.getWins(gameType.name()))), columnX,
    ...
    fontRendererObj
        .drawString(C.BOLD + "Join date", memberX + memberWidth / 3F * 2, memberStart, -1, true);
    memberStart += 10;
    ...
    for (HypixelApiGuild.GuildPlayer guildPlayer : inOrder) {
      ...
      fontRendererObj.drawString(HypixelApiPlayer.DMYHHMMSS.format(guildPlayer.getJoinLong()),
          memberX + memberWidth / 3F * 2, memberStart, -1, true);
      memberStart += 10;
    }
  }
}
