...
public abstract class AbstractHypixelStats {
  ...
  public List<StatsDisplayItem> getQuests(HypixelApiPlayer player) {
    ...
    process(player, statsDisplayItems,
        quests.optJSONObject("quests").optJSONArray(getGameType().getQuestName()));
    return statsDisplayItems;
  }

  public void process(HypixelApiPlayer player, ArrayList<StatsDisplayItem> statsDisplayItems,
      JsonArray ob) {
    for (JsonElement jsonElement : ob) {
      ...
      StringBuilder tmp = new StringBuilder(
          HypixelAPI.INSTANCE.getFrontendNameOfQuest(quest_backend));
      //TODO get quest names

      JsonHolder playerQuestData = player.getQuests()
          .optJSONObject(HypixelAPI.INSTANCE.getFrontendNameOfQuest(quest_backend));
      long last_completed = playerQuestData.optLong("last_completed");
      tmp.append(": ");

      JsonArray requirements = quest.optJSONArray("requirements");
      boolean daily =
          requirements.size() > 0 && new JsonHolder(requirements.get(0).getAsJsonObject())
              .optString("type")
  ...
  public List<StatsDisplayItem> getPreview(HypixelApiPlayer player) {
    ...
    if (gameType == null) {
      ...
      items.add(new DisplayLine(
          ...
          Color.WHITE.getRGB()));
      items.add(new DisplayLine(
          bold("W/L: ", WebsiteUtils.buildRatio(stats.optInt("wins"), stats.optInt("losses"))),
          Color.WHITE.getRGB()));
    }
    return items;
  }
