...
public class AsyncScreenshotSaver implements Runnable {
   ...
   */
  public AsyncScreenshotSaver(final int width, final int height, final int[] pixelValues,
      final Framebuffer frameBuffer, final File screenshotDir, final boolean upload) {
    this.width = width;
    this.height = height;
   ...
   */
  private static void processPixelValues(final int[] pixels, final int displayWidth,
  ...
  public void run() {
    ...
    try {
      ...
      if (OpenGlHelper.isFramebufferEnabled()) {
        ...
        for (int heightSize = texHeight =
            frameBuffer.framebufferTextureHeight - frameBuffer.framebufferHeight;
          ...
          for (int widthSize = 0; widthSize < frameBuffer.framebufferWidth; ++widthSize) {
            ...
            image.setRGB(widthSize, texHeight - heightSize,
                pixelValues[texHeight * frameBuffer.framebufferTextureWidth + widthSize]);
          }
        }
      } else {
      ...
      if (!upload) {
        IChatComponent chatComponent = new ChatComponentText(
            ChatColor.RED + "[Hyperium] " + ChatColor.WHITE + "Captured to " + ChatColor.UNDERLINE
                + screenshot.getName());
        chatComponent.getChatStyle().setChatClickEvent(
            new ClickEvent(ClickEvent.Action.OPEN_FILE, screenshot.getCanonicalPath()));
      ...
      Minecraft.getMinecraft().thePlayer
          .addChatMessage(new ChatComponentTranslation("screenshot.failure", e.getMessage()));
    }
  }
}
