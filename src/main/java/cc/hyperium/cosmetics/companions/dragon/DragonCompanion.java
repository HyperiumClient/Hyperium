...
public class DragonCompanion extends AbstractCosmetic {
  ...
  public void renderEntities(RenderEntitiesEvent entitiesEvent) {
    if (Settings.SHOW_COMPANION_IN_1ST_PERSON) {
      renderPlayer(new RenderPlayerEvent(Minecraft.getMinecraft().thePlayer,
          Minecraft.getMinecraft().getRenderManager(), 0, 0, 0,
  ...
  public void renderPlayer(RenderPlayerEvent event) {
    if (Minecraft.getMinecraft().theWorld == null || !isPurchasedBy(
    ...
    if (packageIfReady == null || packageIfReady.getCachedSettings().getCurrentCompanion()
        != EnumPurchaseType.DRAGON_COMPANION) {
      return;
    }
  ...
  public void tick() {
    ...
    for (Map.Entry<EntityPlayer, CustomDragon> entry : dragonHashMap.entrySet()) {
      ...
      if (entityDragon != null) {
        ...
        double v = animationState.next
            .distanceSqTo(new AnimationPoint(player.posX, player.posY, player.posZ));
        if (v > 7 * 7) {
  ...
  static class AnimationState {
    ...
    void switchToNext(EntityPlayer player, boolean toofar) {
      if (nextNext == null) {
        nextNext = toofar ? new AnimationPoint(player.posX, player.posY + 3, player.posZ)
