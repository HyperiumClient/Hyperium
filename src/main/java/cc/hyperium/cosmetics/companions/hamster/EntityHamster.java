  ...
  public void moveEntityWithHeading(float strafe, float forward) {
    if (!isInWater()) {
      if (!isInLava()) {
        ...
        if (onGround) {
          f4 = worldObj.getBlockState(new BlockPos(MathHelper.floor_double(posX),
        ...
        if (worldObj.isRemote && (!worldObj.isBlockLoaded(new BlockPos((int) posX, 0, (int) posZ))
            ||
            !worldObj.getChunkFromBlockCoords(new BlockPos((int) posX, 0, (int) posZ))
                .isLoaded())) {
        ...
        if (isCollidedHorizontally && isOffsetPositionInLiquid(motionX,
            motionY + 0.6000000238418579D - posY + d1, motionZ)) {
      ...
      if (isCollidedHorizontally && isOffsetPositionInLiquid(motionX,
          motionY + 0.6000000238418579D - posY + d0, motionZ)) {
        motionY = 0.30000001192092896D;
      }
    }
