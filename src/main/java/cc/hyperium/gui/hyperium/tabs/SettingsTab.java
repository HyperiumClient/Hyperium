...
public class SettingsTab extends AbstractTab {
  ...
  public SettingsTab(HyperiumMainGui gui) {
    ...
    for (Object o : gui.getSettingsObjects()) {
      for (Field f : o.getClass().getDeclaredFields()) {
        ...
        boolean mods = false;
        if (ts != null) {
          tabComponent = new ToggleComponent(this, Collections.emptyList(), I18n.format(ts.name()),
              f, o);
    ...
    // Link to capes GUI.
    apply(new LinkComponent(this, Collections.emptyList(), "Youtuber Capes", new CapesGui()), false,
        Category.COSMETICS, items);

    // Link to sidebar mod's gui
    apply(new LinkComponent(this, Collections.emptyList(), "Sidebar Mod",
            new GuiScreenSettings(Hyperium.INSTANCE.getModIntegration().getSidebarAddon())), true,
        Category.OTHER, items);

    // Link to keystrokes gui
    apply(new LinkComponent(this, Collections.emptyList(), "Keystrokes",
            new GuiScreenKeystrokes(Hyperium.INSTANCE.getModIntegration().getKeystrokesMod())), true,
        Category.OTHER, items);

    // Link to togglechat gui
    apply(new LinkComponent(this, Collections.emptyList(), "Togglechat",
            new ToggleChatMainGui(Hyperium.INSTANCE.getModIntegration().getToggleChat(), 0)), true,
        Category.OTHER, items);
  ...
  private void apply(AbstractTabComponent component, boolean mods, Category category,
      HashMap<Category, CollapsibleTabComponent> items) {
    ...
    if (mods) {
      ...
      if (!b) {
        CollapsibleTabComponent c = new CollapsibleTabComponent(this, Collections.emptyList(),
            category.getDisplay());
