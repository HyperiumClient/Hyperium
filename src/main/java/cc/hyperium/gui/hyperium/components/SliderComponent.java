...
public class SliderComponent extends AbstractTabComponent {
  ...
  public SliderComponent(AbstractTab tab, List<String> tags, String label, Field field,
      Object parentObj, float minVal, float maxVal, boolean isInteger, boolean round) {
    super(tab, tags);
    tag(label);
    this.label = label;
    this.field = field;
  ...
  public void render(int x, int y, int width, int mouseX, int mouseY) {
    ...
    lines.clear();
    lines = font
        .splitString(label, (width) / 4); //16 for icon, 3 for render offset and then some more
    ...
    for (String line : lines) {
      font.drawString(line.replaceAll("_", " ").toUpperCase(Locale.ENGLISH), x + 3,
          y + 5 + 17 * line1, 0xffffff);
      line1++;
    }

    int left = x + width / 2;
    String s =
        isInteger ? Integer.toString((int) currentValue) : String.format("%.2f", currentValue);
