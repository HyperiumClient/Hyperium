...
public class CollapsibleTabComponent extends AbstractTabComponent {
  ...
  public void render(int x, int y, int width, int mouseX, int mouseY) {
    ...
    tab.gui.getFont()
        .drawString(label.replaceAll("_", " ").toUpperCase(Locale.ENGLISH), x + 3, y + 5, 0xffffff);
    ...
    for (AbstractTabComponent comp :
        tmpf == null || label.toLowerCase(Locale.ROOT).startsWith(tmpf.toLowerCase(Locale.ROOT)) ?
            children :
      ...
      if (mouseX >= (x1) && mouseX <= (x1) + (parent != null ? width : width / 2) && mouseY >= y
          && mouseY <= y + comp.getHeight() - 2) {
        comp.hover = true;
        comp.mouseEvent(right ? mouseX - width / 2 - x : mouseX - x,
  ...
  public int getHeight() {
    if (collapsed) {
      ...
      List<AbstractTabComponent> children = tmpf == null ? this.children
          : this.children.stream().filter(c -> c.filter(tmpf)).collect(Collectors.toList());
      if (parent != null) {
        int h = 18;
  ...
  public boolean filter(String s) {
    ...
    tmpf = s;
    return b || label.toLowerCase(Locale.ROOT).startsWith(s.toLowerCase(Locale.ROOT)) || children
        .stream().anyMatch(c -> c.filter(s));
  }
