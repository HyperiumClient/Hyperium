...
public class CapesGui extends HyperiumGui implements GuiYesNoCallback {
    ...
    reg("RESET", new GuiButton(nextId(), 1, 1, "Disable Hyperium Cape"), guiButton -> {
      ...
      if (client != null) {
        client.write(ServerCrossDataPacket.build(
            new JsonHolder().put("internal", true).put("set_cape", true).put("value", "default")));
      }
      ...
      if (self == null) {
        GeneralChatHandler.instance()
            .sendMessage("Unable to reset your cape: Your profile is not loaded");
        return;
      }
      ...
      Hyperium.INSTANCE.getHandlers().getGeneralChatHandler()
          .sendMessage("You may need to switch worlds to update your cape.");
    }, guiButton -> {
    });
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    try {
      ...
      int totalRows = (capeAtlas.getKeys().size() / blocksPerLine + (
          capeAtlas.getKeys().size() % blocksPerLine == 0 ? 0 : 1));
      int row = 0;
      int pos = 1;
      int printY = 15 - offset;

      GlStateManager.scale(2F, 2F, 2F);
      fontRendererObj.drawString("Capes",
          (current.getScaledWidth() / 2F - fontRendererObj.getStringWidth("Capes")) / 2,
      ...
      fontRendererObj.drawString(text,
          (current.getScaledWidth() / 2F - fontRendererObj.getStringWidth(text)) / 2,
          (printY + 20) / 2F, new Color(249, 99, 0).getRGB(), true);
      ...
      RenderUtils
          .drawSmoothRect(scaledWidth / 2 - (blockWidth + 16) * blocksPerLine / 2, printY - 4,
      ...
      for (String s : capeAtlas.getKeys()) {
        ...
        int thisBlocksCenter = (int) (scaledWidth / 2 - ((blocksPerLine / 2) - pos + .5) * (
            blockWidth + 16));
        int thisTopY = printY + row * (16 + blockHeight);
        RenderUtils.drawSmoothRect(thisBlocksCenter - blockWidth / 2, thisTopY,
            (thisBlocksCenter + blockWidth / 2), thisTopY + blockHeight, -1);
        ...
        fontRendererObj
            .drawString(nameCape, x / 2F, (thisTopY - 8 + blockHeight / 2F + 64 + 16) / 2,
                new Color(249, 99, 0).getRGB(), true);
        GlStateManager.scale(.5F, .5F, .5F);

        if (cosmeticCallback.getKeys().size() == 0 || purchasing) {
          String string = "Loading";
          fontRendererObj
              .drawString(string, thisBlocksCenter - fontRendererObj.getStringWidth(string),
                  (thisTopY - 8 +
        ...
        if (purchased) {
          ...
          fontRendererObj
              .drawString(string, leftThing3, topThing3, new Color(41, 249, 18).getRGB(), true);
          ...
          if (s.equalsIgnoreCase(s1)) {
            ...
            GlStateManager.scale(2F, 2F, 2F);
            fontRendererObj
                .drawString(string, i / 2F, i1 / 2F, new Color(249, 55, 241).getRGB(), true);
          } else {
            int stringWidth = fontRendererObj.getStringWidth(string);
            int i = thisBlocksCenter - stringWidth;
            int i1 = thisTopY - 8 + blockHeight / 2 + 64 + 48;
            ...
            actions.put(block, () -> {
              JsonHolder purchaseSettings = PurchaseApi.getInstance().getSelf()
                  .getPurchaseSettings();
              ...
              if (client != null) {
                client.write(ServerCrossDataPacket.build(
                    new JsonHolder().put("internal", true).put("set_cape", true).put("value", s)));
              }
          ...
          if (jsonHolder.optBoolean("enough")) {
            ...
            actions.put(block, () -> {
              ...
              ids.put(integer, () -> {
                ...
                if (client != null) {
                  client.write(ServerCrossDataPacket.build(
                      new JsonHolder().put("internal", true).put("cosmetic_purchase", true)
                          .put("value", s)));
                }
              });
            });
  ...
  public void updatePurchases() {
    Multithreading.runAsync(() -> {
      cosmeticCallback = PurchaseApi.getInstance().get(
          "https://api.hyperium.cc/cosmetics/" + Objects.requireNonNull(UUIDUtil.getClientUUID())
              .toString()
              .replace("-", ""));
      purchasing = false;
    });
