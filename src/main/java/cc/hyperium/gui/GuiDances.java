...
public class GuiDances extends HyperiumGui {
  ...
  public GuiDances() {
    ...
    this.handlers.put("Floss", (netty) -> {
      ...
      abstractAnimationHandler.get(Minecraft.getMinecraft().thePlayer.getUniqueID())
          .ensureAnimationFor(seconds);
      NettyClient client = NettyClient.getClient();

      if (client != null && netty) {
        client.write(ServerCrossDataPacket
            .build(new JsonHolder().put("type", "floss_update").put("flossing", true)));
        Multithreading.runAsync(() -> {
          ...
          client.write(ServerCrossDataPacket
              .build(new JsonHolder().put("type", "floss_update").put("flossing", false)));
    ...
    this.handlers.put("Twerk", (netty) -> {
      ...
      handlers.getTwerkDance().getStates()
    ...
    if (Hyperium.INSTANCE.getCosmetics().getFlipCosmetic().isSelfUnlocked()) {
      this.handlers.put("Flip", (netty) -> {
        ...
        Multithreading.runAsync(() -> {
          ...
          if (client != null && netty) {
            client.write(ServerCrossDataPacket
                .build(new JsonHolder().put("type", "flip_update").put("flip_state", 0)));
          }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    for (String s : handlers.keySet()) {
      ...
      GlStateManager.color(tmp.getRed() / 255F, tmp.getGreen() / 255F, tmp.getBlue() / 255F,
          tmp.getAlpha() / 255F);
      ...
      for (String string : strings) {
        textCenterY += 15;
        drawScaledText(string, textCenterX, textCenterY, 1.5F,
            hovered ? Color.YELLOW.getRGB() : Color.decode("#f5f5f5").getRGB(), true, true);
      }
