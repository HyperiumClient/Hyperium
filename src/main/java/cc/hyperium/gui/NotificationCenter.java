...
public class NotificationCenter extends Gui {
   ...
   */
  public Notification display(String title, String description, float seconds,
      @Nullable BufferedImage img, @Nullable Runnable callback, @Nullable Color highlightColor) {
    Notification notif = new Notification(title, description, (int) (seconds * 20), img, callback,
        highlightColor);
  ...
  public class Notification {
    ...
    /**
     * Title text displayed for this notification Max lines is always 1
     */
    ...
    /**
     * What to scale the image to Should be around <code>height / imgSize</code>
     */
    ...
    /**
     * Create a new notification Use {@link NotificationCenter#display} if you wish to create a
     * notification.
     *
     * @param title           Title of the notification
     * @param description     Description of the notification
     ...
     */
    Notification(String title, String description, int ticks, BufferedImage img,
        Runnable clickedCallback, Color highlightColor) {
    ...
    /**
     * Set the description text for this notification Also readjusts the height of notifications
     *
    ...
    void adjustHeight() {
      ...
      int totalHeight =
          (fontRenderer.FONT_HEIGHT + lineSpacing) * (Math.min(maxDescriptionLines, lineCount) + 1)
              + topPadding;
    ...
    private int getWrapWidth() {
      int descRightMargins = (img == null ? rightMargins
          : rightMargins << 1); // Double right margins if image is there
      // Width that text is permitted to stretch across before wrapping/stopping
      return (int) (width - descRightMargins - imgSize * imgScale - highlightBarMargins
          - highlightBarWidth);
    }
    ...
    void render() {
      ...
      fontRenderer.drawString(trimString(String.valueOf(title), width - rightMargins,
          null, true), x + highlightBarWidth + highlightBarMargins, y + topPadding,
          0xFFFFFF | alpha << 24);

      // Description text
      if (descriptionColor == null) {
        descriptionColor = new Color(80, 80, 80); // Anti-NPE
      }
      // Don't draw if no lines
      int wrapWidth = getWrapWidth();
      // Trim & split into multiple lines
      List<String> lines = fontRenderer
          .listFormattedStringToWidth(String.valueOf(description), wrapWidth);
      if (lines.size() > maxDescriptionLines) { // Trim size & last line if overflow
        String nextLine = lines
            .get(maxDescriptionLines); // The line that would appear after the last one
      ...
      for (String line : lines) {
        fontRenderer.drawString(String.valueOf(line),
            ...
            y + topPadding + fontRenderer.FONT_HEIGHT + lineSpacing
                + fontRenderer.FONT_HEIGHT * currentLine,
            descriptionColor.getRGB() | alpha << 24);

        if (++currentLine >= maxDescriptionLines) {
          break; // stop if too many lines have gone by
        }
      }
      ...
      if (img != null) {
        imgScale =
            (double) (height - topPadding - fontRenderer.FONT_HEIGHT - imgTopMargins) / imgSize;
        ...
        drawTexturedModalRect(
            ...
            (float) (y / imgScale
                + (((height + fontRenderer.FONT_HEIGHT) / imgScale) - imgSize) / 2),
            0,
            0,
            imgSize,
            imgSize);
