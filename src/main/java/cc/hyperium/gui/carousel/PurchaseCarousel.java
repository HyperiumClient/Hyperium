...
public class PurchaseCarousel {
  ...
  public void render(int centerX, int centerY, int mouseX, int mouseY) {
    ...
    RenderUtils.drawSmoothRect(objLeft, centerY - mainHeight / 2, objRight, objBottom, 10,
        new Color(23, 23, 23, 255).getRGB());
    ...
    fr.drawString(item.isPurchased() ? I18n.format("gui.purchase.purchased")
            : I18n.format("gui.purchase.purchase"),
        (objLeft + 5) * 3, (objBottom - 20) * 3, new Color(23, 23, 23, 255).getRGB());

    GlStateManager.scale(3 / 2F, 3 / 2f, 3 / 2F);

    RenderUtils.drawFilledCircle(purchaseRight + (barHeight << 1), (objBottom - 12) << 1, barHeight,
        Color.WHITE.getRGB());
    ...
    if (mouseX >= objLeft - 8 && mouseX <= objLeft && mouseY >= centerY - 5
        && mouseY <= centerY + 5) {
      if (!lhover) {
        lhover = true;
    ...
    if (mouseX >= objRight && mouseX <= objRight + 8 && mouseY >= centerY - 5
        && mouseY <= centerY + 5) {
      if (!rhover) {
        rhover = true;
    ...
    GlStateManager.scale(1 / v, 1 / v, 1 / v);
    String s = I18n.format("gui.purchase.state") + ": " + (getCurrent().isPurchased() ? (
        getCurrent().isActive() ?
            I18n.format("gui.purchase.active") : I18n.format("gui.purchase.inactive"))
        : I18n.format("gui.purchase.notpurchased"));
    float e = .5F;

    GlStateManager.scale(e, e, e);
    fr.drawString(s, (centerX - fr.getWidth(s) / 4) / e, (centerY - mainHeight / 2f + 15) / e,
        Color.GREEN.getRGB());
