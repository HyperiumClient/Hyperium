...
public class MaterialTextField {
   ...
   */
  public MaterialTextField(int x, int y, int width, int height, String hint,
      HyperiumFontRenderer fr) {
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
  ...
  public void render(int mx, int my) {
    ...
    Gui.drawRect(x, y + height - 1, x + width, y + height,
        hover || focused ? 0xffffffff : 0xff969696);
    boolean em = text.isEmpty();
    fr.drawString(em && !focused ? hint : text, x, y + height / 2f - fr.FONT_HEIGHT / 2f + 2.5f,
        em ? 0xff969696 : 0xffffffff);
