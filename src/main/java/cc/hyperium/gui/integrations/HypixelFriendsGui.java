...
public class HypixelFriendsGui extends HyperiumGui {
  ...
  protected void pack() {
    ...
    int height = 20;
    if (textField == null) {
      textField = new GuiTextField(nextId(), Minecraft.getMinecraft().fontRendererObj,
          ResolutionUtil.current().getScaledWidth() / 2 - textWidth / 2, 25, textWidth, height);
    }

    reg("SORT",
        new GuiButton(nextId(), ResolutionUtil.current().getScaledWidth() - 153, 23, 150, 20,
            "Sort by: "), guiButton -> {
    ...
    reg("PARTY",
        new GuiButton(nextId(), ResolutionUtil.current().getScaledWidth() - 153, 23 + 21, 150, 20,
            "Party Selected"), guiButton -> {
          Iterator<HypixelApiFriendObject> iterator = selected.iterator();
          while (iterator.hasNext()) {
            HypixelApiFriendObject next = iterator.next();
            if (iterator.hasNext()) {
              Hyperium.INSTANCE.getHandlers().getCommandQueue()
                  .queue("/party invite " + next.getName());
            } else {
              Hyperium.INSTANCE.getHandlers().getCommandQueue()
                  .register("/party invite " + next.getName(), () -> guiButton.enabled = true);
            }
          }
          selected.clear();
          ...
          try {
            if (selected.size() > 10 && !Hyperium.INSTANCE.getHandlers().getDataHandler()
                .getCurrentUser().get().isStaffOrYT()) {
              guiButton.enabled = false;
    ...
    reg("REMOVE",
        new GuiButton(nextId(), ResolutionUtil.current().getScaledWidth() - 153, 23 + (21 << 1),
            150, 20, "Remove (Hold down)"), guiButton -> {
          ...
          if (guiButton.isMouseOver() && Mouse.isButtonDown(0) && guiButton.enabled) {
            ...
            if (removeTicks >= totalTick) {
              ...
              while (iterator.hasNext()) {
                ...
                if (iterator.hasNext()) {
                  Hyperium.INSTANCE.getHandlers().getCommandQueue()
                      .queue("/friend remove " + next.getName());
                } else {
                  Hyperium.INSTANCE.getHandlers().getCommandQueue()
                      .register("/friend remove " + next.getName(), () -> guiButton.enabled = true);
                }
              }
              guiButton.enabled = false;
              selected.clear();
            ...
            guiButton.displayString =
                ChatColor.RED + "Removing in: " + NumberUtil.round(remaining / 20, 1);
    ...
    for (GuiBoxItem<HypixelApiFriendObject> selectedBox : friendListBoxes) {
      if (selectedBox.getBox().isMouseOver(mouseX, mouseY)) {
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
          ...
          GuiBoxItem<HypixelApiFriendObject> e = new GuiBoxItem<>(
              new GuiBlock(2 + 5, columnWidth + 5, topRenderBound + 1 + (selected.size()) * 11,
                  topRenderBound + 1 + (selected.size() + 1) * 11), selectedBox.getObject());
          selectedBoxes.add(e);
          selectedItem = e;
        } else {
          selectedItem = selectedBox;
  ...
  private void rebuildFriends() {
    try {
      this.friends = new HypixelFriends(
          Hyperium.INSTANCE.getHandlers().getDataHandler().getFriendsForCurrentUser().get()
              .getData());
    } catch (InterruptedException | ExecutionException e) {
      e.printStackTrace();
    }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    friends.removeIf(
        hypixelApiFriendObject -> !hypixelApiFriendObject.getDisplay().toLowerCase(Locale.ENGLISH)
            .contains(textField.getText().toLowerCase(Locale.ENGLISH)));
    ...
    if (selectedItem != null) {
      ...
      int right =
          box.getLeft() + fontRendererObj.getStringWidth(selectedItem.getObject().getDisplay()) + 2;
    ...
    int row = 1;
    namesBlock
        .drawString("Currently selected: ", fontRendererObj, false, true, namesBlock.getWidth() / 2,
            1, true, true, Color.RED.getRGB(), true);
    for (HypixelApiFriendObject object : selected) {
      namesBlock
          .drawString(object.getDisplay(), fontRendererObj, false, false, 5, 1 + row * 11, true,
              true, Color.WHITE.getRGB(), true);
      selectedBoxes.add(new GuiBoxItem<>(
          new GuiBlock(2 + 5, namesBlock.getRight() + 5, namesBlock.getTop() + 1 + row * 11,
              namesBlock.getTop() + 1 + (row + 1) * 11), object));
      row++;
    }
    GuiBlock friendsBlock = new GuiBlock(namesBlock.getRight() + 15,
        ResolutionUtil.current().getScaledWidth() - 100, topRenderBound, bottomRenderBound);
    ...
    for (HypixelApiFriendObject object : friends.get()) {
      ...
      if (selectedItem != null && selectedItem.getObject().equals(object) && !selected
          .contains(selectedItem.getObject())) {
        selectedItem = new GuiBoxItem<>(
            new GuiBlock(drawX, drawX + columnWidth, drawY + friendsBlock.getTop(),
                drawY + friendsBlock.getTop() + 11), object);
      }
      if (friendsBlock.drawString(object.getDisplay(), fontRendererObj, false, false,
          drawX - friendsBlock.getLeft(), drawY, false, false, Color.WHITE.getRGB(), true)) {
        GuiBoxItem<HypixelApiFriendObject> e = new GuiBoxItem<>(
            new GuiBlock(drawX, drawX + columnWidth, drawY + friendsBlock.getTop(),
                drawY + friendsBlock.getTop() + 11), object);
  ...
  public void onRemove(FriendRemoveEvent event) {
    ...
    try {
      friends = Hyperium.INSTANCE.getHandlers().getDataHandler().getFriendsForCurrentUser().get()
          .getData();
    } catch (InterruptedException | ExecutionException e) {
      e.printStackTrace();
    }
    ...
    for (Map.Entry<String, JsonElement> stringJsonElementEntry : friends.getObject().entrySet()) {
      ...
      String display = stringJsonElementEntry.getValue().getAsJsonObject().get("display")
          .getAsString();
