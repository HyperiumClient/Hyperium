...
public class GuiHyperiumCredits extends HyperiumGui {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    drawCenteredString(fr,
        err.isEmpty() ? (contributors.isEmpty() ? "Loading contributors..." : "Contributors")
            : "Error: " + err, width / 4, 20 + offY / 2, -1);
    ...
    for (Map.Entry<String, DynamicTexture> entry : contributors.entrySet()) {
      ...
      drawString(fr, commits.get(contributorStatistic) + " commits", x + 25, y.get() + 10,
          0x757575);
      y.addAndGet(25);
    }
  }
  ...
  public void handleMouseInput() throws IOException {
    ...
    if (i < 0 && offY > -240) {
      offY -= 20; // todo: not hardcode this
    } else if (i > 0 && offY < 0) {
      offY += 20;
    }
  }
  ...
  private static void fetch(int tries) {
    try {
      String content = IOUtils.toString(
          URI.create("https://api.github.com/repos/HyperiumClient/Hyperium/stats/contributors"),
          StandardCharsets.UTF_8);
      JsonParser parser = new JsonParser();
      JsonArray a = parser.parse(content).getAsJsonArray();
      StreamSupport.stream(a.spliterator(), false).map(JsonElement::getAsJsonObject)
          .filter(o -> o.has("total") &
              o.get("total").getAsInt() > 20).sorted(
          Comparator.comparingLong(o -> ((JsonObject) o).get("total").getAsInt()).reversed())
          .forEach(o -> {
            JsonObject con = o.get("author").getAsJsonObject();

            try {
              BufferedImage bi = ImageIO
                  .read(new URL(con.get("avatar_url").getAsString() + "&size=64"));
