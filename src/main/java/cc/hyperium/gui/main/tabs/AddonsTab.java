...
public class AddonsTab extends AbstractTab {
  ...
  public AddonsTab(int y, int w) {
    ...
    items.add(new SettingItem(
        () -> HyperiumMainGui.INSTANCE.setTab(HyperiumMainGui.INSTANCE.getTabs().get(6)),
        Icons.DOWNLOAD.getResource(), "Addons", "Download addons",
        "Click to open menu to download addons", xi, yi));
  }
  ...
  @Override
  public void draw(int mouseX, int mouseY, int topX, int topY, int containerWidth,
      int containerHeight) {
