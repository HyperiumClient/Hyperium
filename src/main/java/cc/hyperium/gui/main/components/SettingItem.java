...
*/
    ...
    public void mouseClicked(int mouseX, int mouseY) {
        if (MainHyperiumMainGui.INSTANCE.getOverlay() != null) return;
        if (System.currentTimeMillis() - MainHyperiumMainGui.INSTANCE.getLastSelectionChange() < 100) return;
        this.clickX = mouseX;
        this.clickY = mouseY;
    }
    ...
    public void render(int mouseX, int mouseY, int containerWidth, int containerHeight, int topX, int topY) {
        ...
        if (mouseX >= blockX && mouseX <= blockX + w / 7 * 5 && mouseY >= blockY && mouseY <= bottom && MainHyperiumMainGui.INSTANCE.getOverlay() == null) {
        ...
        MainHyperiumMainGui.INSTANCE.getFr().drawString(title, blockX + 3, blockY + 3, 0xffffff);
        float s = 0.8f;
        if (desc != null) {
            ...
            for (String word : desc.split(" ")) {
                if (word.equalsIgnoreCase("\n")) {
                    MainHyperiumMainGui.INSTANCE.getFr().drawString(tmp.toString(), (blockX + 3) / s, (blockY + offsetY) / s, new Color(160, 160, 160).getRGB());
                    offsetY += 10;
                ...
                if (MainHyperiumMainGui.INSTANCE.getFr().getWidth(tmp + word) * s < w / 7 * 5 - (offsetY >= 25 ? 0 : 27))
        ...
        if (mouseX >= blockX + w / 7 * 5 - 10 && mouseX <= blockX + w / 7 * 5 && mouseY >= blockY + h / 7 * 5 - 10 && mouseY < blockY + h / 7 * 5 && MainHyperiumMainGui.INSTANCE.getOverlay() == null) {
            HyperiumGui.drawChromaBox(mouseX + 10, mouseY, (int) (mouseX + 10 + MainHyperiumMainGui.INSTANCE.getFr().getWidth(hover) * s + 3), (int) (mouseY + MainHyperiumMainGui.INSTANCE.getFr().FONT_HEIGHT * s + 3), 0.2f);
            GlStateManager.scale(s, s, s);
            MainHyperiumMainGui.INSTANCE.getFr().drawString(hover, (mouseX + 10) / s + 1, mouseY / s + 1, 0xffffff);
            GlStateManager.scale(1.25f, 1.25f, 1.25f);
        }
        this.clickY = -1;
        this.clickX = -1;
