...
public class OverlayToggle extends OverlayComponent {
  ...
  public boolean render(int mouseX, int mouseY, int overlayX, int overlayY, int w, int h,
    ...
    if (enabled) {
      ...
      int color = GraphicsUtil.INSTANCE
          .transitionOfHueRange(colorStep, 1, getHue(136, 255, 0), 0.7F, 0.7F).getRGB();
  ...
  public void mouseClicked(int mouseX, int mouseY, int overlayX, int overlayY, int w, int h) {
    ...
    if (mouseX >= overlayX + w - 30 && mouseX <= overlayX + w - 5 && mouseY >= overlayY + 5
        && mouseY <= overlayY + h - 5) {
      toggle = !toggle;
      if (callback != null) {
  ...
  private int getHue(int red, int green, int blue) {
    ...
    if (max == red) {
      ...
      hue = max == green ? 2f + (blue - red) / (max - min) : 4f + (red - green) / (max - min);
    }
