...
public class SplashProgress {
  ...
  public static void drawSplash(TextureManager tm) {
    ...
    GlStateManager
        .ortho(0.0D, scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(), 0.0D,
            1000.0D, 3000.0D);
    ...
    if (splash == null) {
      splash = new ResourceLocation("hyperium", "textures/hyperium-splash.png");
    }
    ...
    framebuffer.framebufferRender(scaledresolution.getScaledWidth() * scaleFactor,
  ...
  private static void drawProgress() {
    if (Minecraft.getMinecraft().gameSettings == null
        || Minecraft.getMinecraft().getTextureManager() == null) {
      return;
    }

    // Declare the font to be used
    if (raleway == null) {
      raleway = new HyperiumFontRenderer("Raleway", 20);
    }
    if (roboto == null) {
      roboto = new HyperiumFontRenderer("Roboto", 20);
    }
    ...
    roboto.drawString(s, sr.getScaledWidth() - 20 - roboto.getWidth(s), sr.getScaledHeight() - 25,
        -1);

    // Render the blue progress bar
    Gui.drawRect(0, sr.getScaledHeight() - 2, (int) calc, sr.getScaledHeight(),
