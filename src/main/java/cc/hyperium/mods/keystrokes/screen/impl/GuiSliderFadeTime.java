...
public class GuiSliderFadeTime extends GuiSlider {
  ...
  public GuiSliderFadeTime(KeystrokesMod mod, int id, int xPos, int yPos, int width, int height,
      GuiScreenKeystrokes keystrokes) {
    super(id, xPos, yPos, width, height, "Fade Time: ", "%", 0, 30,
        mod.getSettings().getFadeTime() * 100.0D, false, true);
  ...
  private void updateDisplayString() {
    displayString = getValue() < 10 ? dispString + "Slow"
        : getValue() > 20 ? dispString + "Fast" : dispString + "Normal";
  }
}
