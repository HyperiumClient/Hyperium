...
public class GuiScreenEditKeys extends GuiScreen {
  ...
  public void onGuiClosed() {
    ...
    Hyperium.INSTANCE.getHandlers().getGuiDisplayHandler()
        .setDisplayNextTick(new GuiScreenKeystrokes(mod));
  }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (selected != null) {
      ...
      drawRect(hitbox.getLeft(), hitbox.getTop(), hitbox.getRight(), hitbox.getBottom(),
          Color.WHITE.getRGB());
    ...
    if (selected != null && Mouse.isButtonDown(0) && currentlyDragging == null &&
        selected.getKey().getHitbox().multiply(mod.getSettings().getScale())
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    for (CustomKeyWrapper wrapper : getKeys()) {
      if (wrapper.getKey().getHitbox().multiply(mod.getSettings().getScale())
          .isMouseOver(mouseX, mouseY)) {
        selected = wrapper;
        lastMouseX = mouseX;
        lastMouseY = mouseY;
  ...
  protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton,
    ...
    if (currentlyDragging != null) {
      ...
      currentlyDragging
          .setxOffset(currentlyDragging.getxOffset() + (mouseX - lastMouseX) / settings.getScale());
      currentlyDragging
          .setyOffset(currentlyDragging.getyOffset() + (mouseY - lastMouseY) / settings.getScale());
      lastMouseX = mouseX;
      lastMouseY = mouseY;
      updated = true;
    }
