...
public class MouseButton extends AbstractKey {
  ...
  public void renderKey(int x, int y) {
    ...
    int colorN = new Color(0, 0, 0).getRGB() + ((int) ((double) red * textBrightness) << 16) + (
        (int) ((double) green * textBrightness) << 8) + (int) ((double) blue * textBrightness);
    ...
    if (mod.getSettings().isShowingCPSOnButtons() && mod.getSettings().isShowingCPS()) {
      ...
      if (mod.getSettings().isChroma()) {
        ...
        drawChromaString((name.equals(BUTTONS[0]) ? mod.getRenderer().getCPSKeys()[0].getLeftCPS() :
                mod.getRenderer().getCPSKeys()[0].getRightCPS()) + " CPS",
            Math.round(x / 0.5f + xOffset / 0.5f + 10 / 0.5f), round, .5);
      } else {
        mc.fontRendererObj
            .drawString(name, x + xOffset + 8, y + yOffset + 4, pressed ? pressedColor : colorN);
        GL11.glPushMatrix();
        GL11.glScalef(0.5f, 0.5f, 0.0f);
        mc.fontRendererObj.drawString(
            (name.equals(MouseButton.BUTTONS[0]) ? mod.getRenderer().getCPSKeys()[0].getLeftCPS() :
                mod.getRenderer().getCPSKeys()[0].getRightCPS()) + " CPS",
            Math.round(x / 0.5f + xOffset / 0.5f + 20.0f), round, pressed ? pressedColor : colorN);
      }
      GL11.glPopMatrix();
    } else {
      if (mod.getSettings().isChroma()) {
        ...
        mc.fontRendererObj
            .drawString(name, x + xOffset + 8, y + yOffset + 8, pressed ? pressedColor : colorN);
      }
    }
  }
}
