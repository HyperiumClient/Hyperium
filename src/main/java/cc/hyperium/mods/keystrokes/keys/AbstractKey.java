...
public abstract class AbstractKey extends Gui {
  ...
  protected Color getChromaColor(double x, double y, double offsetScale) {
    ...
    return new Color(Color.HSBtoRGB(
        (float) ((System.currentTimeMillis() - (x * 10) * offsetScale - (y * 10) * offsetScale) % v)
            / v, 0.8F, 0.8F));
  }
  ...
  protected final int getColor() {
    return mod.getSettings().isChroma() ? Color.HSBtoRGB(
        (float) ((System.currentTimeMillis() - (xOffset * 10) - (yOffset * 10)) % 2000) / 2000.0F,
        0.8F, 0.8F) : new Color(mod.getSettings().getRed(), mod.getSettings().getGreen(),
        mod.getSettings().getBlue()).getRGB();
  }
  ...
  protected final int getPressedColor() {
    return mod.getSettings().isChroma() ? new Color(0, 0, 0).getRGB()
        : new Color(mod.getSettings().getPressedRed(), mod.getSettings().getPressedGreen(),
            mod.getSettings().getPressedBlue()).getRGB();
  }

  /**
   * Draws a centered string without a background shadow at the specified location with the given
   * color
   *
  ...
  protected final void drawCenteredString(String text, int x, int y, int color) {
    mc.fontRendererObj
        .drawString(text, (float) (x - mc.fontRendererObj.getStringWidth(text) / 2), (float) y,
            color, false);
  }
