...
public class SpaceKey extends AbstractKey {
  ...
  private boolean isButtonDown(int buttonCode) {
    if (buttonCode < 0) {
      return Mouse.isButtonDown(buttonCode + 100); // Mouse bind.
    } else if (buttonCode > 0) {
      return Keyboard.isKeyDown(buttonCode); // Key bind.
    }
    return false;
  }
  ...
  public void renderKey(int x, int y) {
    ...
    String name = this.name.equalsIgnoreCase("space") ? (mod.getSettings().isChroma() ? "------"
        : (ChatColor.STRIKETHROUGH.toString() + "------")) : "Sneak";
    ...
    if (pressed) {
      color = Math.min(255,
          (int) ((mod.getSettings().getFadeTime() * 5) * (System.currentTimeMillis() - lastPress)));
      textBrightness = Math.max(0.0D, 1.0D - (double) (System.currentTimeMillis() - lastPress) / (
          mod.getSettings().getFadeTime() * 2));
    } else {
      color = Math.max(0,
          255 - (int) ((mod.getSettings().getFadeTime() * 5) * (System.currentTimeMillis()
              - lastPress)));
      textBrightness = Math.min(1.0D,
          (double) (System.currentTimeMillis() - lastPress) / (mod.getSettings().getFadeTime()
              * 2));
    }
    ...
    int colorN = new Color(0, 0, 0).getRGB() + ((int) ((double) red * textBrightness) << 16) + (
        (int) ((double) green * textBrightness) << 8)
    ...
    if (mod.getSettings().isChroma()) {
      if (this.name.equalsIgnoreCase("space")) {
        ...
        drawGradientRect(0, 0, 2, 35, Color
            .HSBtoRGB((float) ((System.currentTimeMillis() - xIn * 10 - y2 * 10) % 2000L) / 2000.0f,
                0.8f, 0.8f), Color.HSBtoRGB(
            (float) ((System.currentTimeMillis() - (xIn + 35) * 10 - y2 * 10) % 2000L) / 2000.0f,
            0.8f, 0.8f));
        GlStateManager.popMatrix();
      } else {
        drawChromaString(name, x + ((xOffset + 70) / 2)
                - Minecraft.getMinecraft().fontRendererObj.getStringWidth(name) / 2, y + yOffset + 5,
            1.0F);
      }
    } else {
