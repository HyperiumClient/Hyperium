  ...
  public void renderKey(int x, int y) {
    ...
    if (pressed) {
      ...
      textBrightness = Math.max(0.0,
          1.0 - (System.currentTimeMillis() - lastPress) / (mod.getSettings().getFadeTime() * 5.0));
    } else {
      color = Math.max(0,
    ...
    if (mod.getSettings().isUsingArrowKeys()) {
      ...
      worldrenderer.pos(centerX, top, 0.0D)
          .color(topColor.getRed(), topColor.getGreen(), topColor.getBlue(), 255).endVertex();
      worldrenderer.pos(centerX, top, 0.0D)
          .color(topColor.getRed(), topColor.getGreen(), topColor.getBlue(), 255).endVertex();
      worldrenderer.pos(left, bottom, 0.0D)
          .color(bottomLeftColor.getRed(), bottomLeftColor.getGreen(), bottomLeftColor.getBlue(),
              255).endVertex();
      worldrenderer.pos(right, bottom, 0.0D)
          .color(bottomRightColor.getRed(), bottomRightColor.getGreen(), bottomLeftColor.getBlue(),
              255).endVertex();
      tessellator.draw();
      ...
      if (mod.getSettings().isChroma()) {
        ...
        mc.fontRendererObj
            .drawString(name, (int) xPos, (int) yPos, pressed ? pressedColor : colorN);
      }
    }
