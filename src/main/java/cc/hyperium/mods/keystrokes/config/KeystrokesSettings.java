...
public class KeystrokesSettings {
  ...
  public void save() {
    try {
      ...
      Hyperium.LOGGER
          .warn(String.format("Could not save config file! (\"%s\")", configFile.getName()));
    }
  }
    ...
    if (data.has("custom")) {
      ...
      for (JsonElement element : custom) {
        ...
        CustomKeyWrapper wrapper = new CustomKeyWrapper(new CustomKey(
            theMod, holder.optInt("key"), holder.optInt("type")), holder.optInt("xOffset"),
            holder.optInt("yOffset"));
