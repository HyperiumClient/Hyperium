...
public class ToggleChatMainGui extends GuiScreen {
  ...
  public void drawScreen(int x, int y, float ticks) {
    ...
    drawCenteredString(fontRendererObj, String.format("Page %s/%s", (pageNumber),
        (int) Math.ceil((double) main.getToggleHandler().getToggles().size() / 7D)), width / 2,
        height / 2 - 94, -1);
    ...
    if (button.id == 0) {
      for (ToggleBase base : main.getToggleHandler().getToggles().values()) {
        if (data.containsKey(button) && base.equals(data.get(button))) {
          base.toggle();
          button.displayString = (String
              .format(base.getDisplayName(), base.getStatus(base.isEnabled())));
          changed = true;
          break;
        }
      }
  ...
  public void onGuiClosed() {
    if (changed) {
      main.getConfigLoader().saveToggles();
    }
          ...
          if (toggleBase.hasDescription()) {
            ...
            for (String text : toggleBase.getDescription()) {
              drawCenteredString(mc.fontRendererObj,
                  ChatColor.translateAlternateColorCodes('&', text), width / 2 + 150, position[0],
                  Color.WHITE.getRGB());
              position[0] += 10;
            }
          }
