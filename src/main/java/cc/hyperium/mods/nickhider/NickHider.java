...
public class NickHider extends AbstractMod {
  ...
  private final Pattern newNick = Pattern
      .compile("We've generated a random username for you: \\s*(?<nick>\\S+)");
  ...
  public AbstractMod init() {
    ...
    Multithreading.runAsync(() -> {
      String s = sk1erMod.rawWithAgent(
          "https://sk1er.club/words.txt?uuid=" + Minecraft.getMinecraft().getSession().getProfile()
              .getId());
      ...
      if (currPage < bookPages.tagCount()) {
        try {
          ...
          if (matcher.find()) {
            ...
            remap(nick,
                override == null ? Minecraft.getMinecraft().getSession().getProfile().getName()
                    : override);
          }
        } catch (Exception e) {
          e.printStackTrace();
  ...
  public void profileCheck(TickEvent event) {
    if (nickHiderConfig.isMasterEnabled()) {
      ...
      for (NetworkPlayerInfo networkPlayerInfo : sendQueue.getPlayerInfoMap()) {
        ...
        if (gameProfile.getId() != null && gameProfile.getId()
            .equals(Minecraft.getMinecraft().getSession().getProfile().getId())) {
          if (!gameProfile.getName()
              .equalsIgnoreCase(Minecraft.getMinecraft().getSession().getProfile().getName())) {
            remap(gameProfile.getName(),
                override == null ? Minecraft.getMinecraft().getSession().getProfile().getName()
                    : override);
          }
  ...
  public void remap(String key, String newKey) {
    ...
    if (usedNicks.contains(key) || key.isEmpty() || key.contains(" ")) {
      return;
    }
    usedNicks.add(key);

    String newName = key.length() > 2 ? newKey : key;
    Nick nick = new Nick(Pattern.compile(Pattern.quote(key), Pattern.CASE_INSENSITIVE), key,
        newName);
    nicks.add(nick);
  ...
  public String apply(String input) {
    ...
    if (!nickHiderConfig.isMasterEnabled() || forceDown || !nickHiderConfig.isHideNames()) {
      return input;
    }
    if (cache.size() > 5000) {
      cache.clear();
  ...
  public String out(String chat) {
    ...
    for (Nick nick : nicks) {
      chat = Pattern.compile(nick.newName, Pattern.CASE_INSENSITIVE).matcher(chat)
          .replaceAll(nick.oldName);
    }
  ...
  public ResourceLocation getPlayerSkin() {
    if (playerSkin == null && !startedLoadingSkin) {
      ...
      Minecraft.getMinecraft().getSkinManager()
          .loadProfileTextures(Minecraft.getMinecraft().getSession().getProfile(),
  ...
  private String getPseudo(String input) {
    ...
    return size == 0 ? "Player-error" : nickHiderConfig.getPrefix() + namesDatabase.get(i % size) +
        nickHiderConfig.getSuffix() + (nickHiderConfig.getPrefix().equalsIgnoreCase("Player-") ? ""
        : getStar());
  }
  ...
  private String getStar() {
    ...
    int i = s.hashCode();
    assert i == 42 :
        "Potential illegal nickhider modification found. If you did NOT edit NickHider, contact Sk1er ("
            + s + ")";
    return s;
  }
