...
public class CommandNickHider implements BaseCommand {
  ...
  public void onExecute(String[] args) {
    ...
    if (args.length == 0) {
      if (instance.isExtendedUse()) {
        sendMessage(
            "Extended use is active. You can access limited commands. /nickhider myname <Alternate name instead of your own> /nickhider set <user> <nick for that user>");
      }
      sendAll();
      ...
      if (args[0].equalsIgnoreCase("clear")) {
        ...
        sendMessage("Set Use Your Real Skin On Other Players: " + (config.isUsePlayerSkinForAll() ?
            ChatColor.GREEN + "Yes" : ChatColor.RED + "No"));
        sendAll();
  ...
  public void sendMessage(String message, String hover, String click) {
    ...
    if (click != null && !click.isEmpty()) {
      chatComponent.getChatStyle()
          .setChatClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, click));
    }
  ...
  private void sendAll() {
    NickHider instance = NickHider.instance;
    NickHiderConfig config = instance.getNickHiderConfig();
    sendMessage("Mod Status: " + (config.isMasterEnabled() ? ChatColor.GREEN + "Enabled"
        : ChatColor.RED + "Disabled"), "Click to toggle the mod", "/nickhider toggle");
    sendMessage("Name Hiding: " + (config.isHideNames() ? ChatColor.GREEN + "Enabled"
        : ChatColor.RED + "Disabled"), "Click to toggle name hiding", "/nickhider names");
    sendMessage("Hide Other Names " + (config.isHideOtherNames() ? ChatColor.GREEN + "Yes"
            : ChatColor.RED + "No"), "Click to toggle hiding other players' names",
        "/nickhider othernames");
    sendMessage("Hide Your Skin: " + (config.isHideSkins() ? ChatColor.GREEN + "Yes"
        : ChatColor.RED + "No"), "Click to toggle hiding skins", "/nickhider skins");
    sendMessage("Hide Other Players' Skins: " + (config.isHideOtherSkins() ? ChatColor.GREEN + "Yes"
        : ChatColor.RED + "No"), "Click to toggle other players's names", "/nickhider otherskins");
    sendMessage(
        "Use Your Real Skin On Yourself: " + (config.isUseRealSkinForSelf() ? ChatColor.GREEN
            + "Yes" : ChatColor.RED + "No"),
        "Click to toggle using your own skin vs the default one", "/nickhider myskin");
    sendMessage(
        "Use Your Real Skin On Other Players " + (config.isUsePlayerSkinForAll() ? ChatColor.GREEN
            + "Yes" : ChatColor.RED + "No"),
        "Click to toggle using your own skin vs the default one", "/nickhider myskinonothers");
    sendMessage("Clear Cache", "Clears the mod's cache of names", "/nickhider clear");
    sendMessage("Change Psuedo: ",
        "Change the psuedo NickHider uses with /nickhider pseudo <pseudo>");
    sendMessage("Change Nick Prefix (Hover)",
        "Change the prefix NickHider uses with /nickhider prefix <prefix>. Use NONE to reset. \nA * will be added to the end of the player name.");
    sendMessage("Change Nick Suffix (Hover)",
        "Change the suffix NickHider uses with /nickhider suffix <suffix>. Use NONE to reset. \nA * will be added to the end of the player name.");
    sendMessage(
        ChatColor.AQUA + ChatColor.BOLD.toString() + "Hover for details, click to execute command");
    sendMessage(syntax);
  }
}
