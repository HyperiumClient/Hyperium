...
public class Levelhead extends AbstractMod {
  ...
  public AbstractMod init() {
    ...
    try {
      jsonHolder = new LevelheadJsonHolder(
          FileUtils.readFileToString(new File(Hyperium.folder, "levelhead.json")));
    } catch (Exception e) {
      e.printStackTrace();
    }
    ...
    Multithreading.runAsync(() -> types = new LevelheadJsonHolder(
        rawWithAgent("https://api.sk1er.club/levelhead_config")));
    ...
    Hyperium.INSTANCE.getHandlers().getCommandHandler()
        .registerCommand(new CustomLevelheadCommand());
  ...
  private String rawWithAgent(String url) {
    ...
    try {
      ...
      connection
          .addRequestProperty("User-Agent", "Mozilla/4.76 (SK1ER LEVEL HEAD V" + VERSION + ")");
  ...
  public void fetch(UUID uuid, LevelheadDisplay display, boolean allowOverride) {
    ...
    if (purchaseStatus.has(type) && !purchaseStatus.optBoolean(type)) {
      ...
      fakeValue.put("strlevel",
          "Item '" + type + "' not purchased. If you believe this is an error, contact Sk1er");
  ...
  private LevelheadTag buildTag(LevelheadJsonHolder object, UUID uuid, LevelheadDisplay display,
    ...
    footerObj.merge(display.getFooterConfig()
            .put("footer", object.optString("strlevel", format.format(object.getInt("level")))),
        !allowOverride);
  ...
  private synchronized void refreshRawPurchases() {
    rawPurchases = new LevelheadJsonHolder(rawWithAgent(
        "https://api.sk1er.club/purchases/" + Minecraft.getMinecraft().getSession().getProfile()
            .getId().toString()));
  }
  ...
  public synchronized void refreshPurchaseStates() {
    purchaseStatus = new LevelheadJsonHolder(rawWithAgent(
        "https://api.sk1er.club/levelhead_purchase_status/" + Minecraft.getMinecraft().getSession()
            .getProfile().getId().toString()));
