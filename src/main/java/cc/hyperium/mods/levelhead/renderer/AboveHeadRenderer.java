...
public class AboveHeadRenderer {
  ...
  public void render(RenderPlayerEvent event) {
    ...
    for (AboveHeadDisplay display : levelhead.getDisplayManager().getAboveHead()) {
      ...
      if (display.loadOrRender(player) && levelheadTag != null
          && !(levelheadTag instanceof NullLevelheadTag)) {
        if ((event.getEntity().getUniqueID().equals(Levelhead.getInstance().userUuid) && !display
            .getConfig().isShowSelf()
        ...
        if (player.getDistanceSqToEntity(Minecraft.getMinecraft().thePlayer) < 64 << 6) {
          ...
          if (objective != null
              && event.getEntity().getDistanceSqToEntity(Minecraft.getMinecraft().thePlayer)
              < 10 * 10 && (event.getEntity() != Minecraft.getMinecraft().thePlayer
              || Settings.SHOW_OWN_NAME)) {
            offset *= 2;
          }
          ...
          renderName(event, levelheadTag, player, event.getX(), event.getY() + offset + o * .3D,
              event.getZ());
        }
      }
  ...
  private void renderName(RenderPlayerEvent event, LevelheadTag tag, EntityPlayer player, double x,
      double y, double z) {
    FontRenderer fontrenderer = event.getRenderManager().getFontRenderer();
    float f = (float) (1.6F * Levelhead.getInstance().getDisplayManager().getMasterConfig()
        .getFontSize());
  ...
  private void render(FontRenderer fontRenderer, LevelheadComponent header, int x) {
    ...
    int y = 0;
    if (header.isRgb()) {
      fontRenderer.drawString(header.getValue(), x, y,
          new Color((float) header.getRed() / 255F, (float) header.getGreen() / 255F,
      ...
      fontRenderer.drawString(header.getColor() + header.getValue(), x, y,
          Color.WHITE.darker().darker().darker().darker().darker().getRGB() * 255);
    }
    ...
    if (header.isRgb()) {
      ...
      fontRenderer.drawString(header.getValue(), x, y,
          new Color(header.getRed(), header.getGreen(), header.getBlue()).getRGB());
    } else if (header.isChroma()) {
      fontRenderer.drawString(header.getValue(), x, y,
          header.isChroma() ? Levelhead.getInstance().getRGBColor() : 553648127);
    } else {
      GlStateManager.color(255, 255, 255, .5F);

      fontRenderer
          .drawString(header.getColor() + header.getValue(), x, y, Color.WHITE.darker().getRGB());
    }
  }
}
