...
public class LevelheadChatRenderer {
  ...
  public static IChatComponent modifyChat(IChatComponent component, String tag,
      DisplayConfig config) {
    ChatComponentText text = new ChatComponentText(
        config.getHeaderColor() + "[" + config.getFooterColor() + tag + config.getHeaderColor()
            + "]" + ChatColor.RESET);
  ...
  public void chat(ServerChatEvent event) {
    ...
    if (chat == null || !levelhead.getLevelheadPurchaseStates().isChat() || !chat.getConfig()
        .isEnabled()) {
      return;
    }
