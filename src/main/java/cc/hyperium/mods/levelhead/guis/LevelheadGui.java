...
public class LevelheadGui extends GuiScreen implements GuiYesNoCallback {
  ...
  public void initGui() {
    ...
    Multithreading.runAsync(() -> {
      String raw = Sk1erMod.getInstance()
          .rawWithAgent("https://api.sk1er.club/levelhead/" + uuid.toString().replace("-", ""));
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    drawScaledText(UNDERLINE.toString() + BOLD + "Levelhead " + instance.getVersion(), width / 2, 5,
        2, -1, true, true);
    drawScaledText("By Sk1er LLC", width / 2, 30, 1.5, -1, true, true);

    reg(new GuiButton(++currentID, 1, 2, 150, 20,
            YELLOW + "Mod Status: " + (instance.getDisplayManager().getMasterConfig().isEnabled() ?
                GREEN + "Enabled" : RED + "Disabled")),
        button -> {
          instance.getDisplayManager().getMasterConfig()
              .setEnabled(!instance.getDisplayManager().getMasterConfig().isEnabled());
          button.displayString =
              "Mod Status: " + (instance.getDisplayManager().getMasterConfig().isEnabled() ? GREEN
    ...
    if (purchasingStats) {
      ...
      drawScaledText("These stats can be displayed above players' heads, in lobbies or in tab.",
          width / 2, 67, 1, -1,
          true, true);
      ...
      drawScaledText(LIGHT_PURPLE + "Levelhead Credits: " + AQUA + instance.getRawPurchases()
              .optInt("remaining_levelhead_credits"),
          width / 3, height - 90,
          1.5D, -1, true, true);

      drawScaledText((currentlyBeingEdited instanceof AboveHeadDisplay ? GREEN
              : (currentlyBeingEdited instanceof TabDisplay &&
                  instance.getLevelheadPurchaseStates().isTab()
                  ? GREEN : (currentlyBeingEdited instanceof ChatDisplay && instance
                  .getLevelheadPurchaseStates().isChat() ? GREEN : RED))) + types[index], width / 3,
      ...
      if (bigChange) {
        ...
        for (String s : "Some changes cannot be applied in real time.\nThey will be applied once you close this GUI"
            .split("\n")) {
          fontRenderer.drawString(s, width - fontRenderer.getStringWidth(s) - 5, start + (i * 10),
              Color.RED.getRGB(), true);
          i++;
        }
      }

      drawScaledText(LIGHT_PURPLE + "Custom Levelhead Status: " + (isCustom ? GREEN + "Enabled"
              : RED + "Disabled"),
          2, height - 55, 1.25, -1,
          true, false);

      reg(new GuiButton(++currentID, 2, height - 44, 220, 20,
          (isCustom ? ChatColor.YELLOW + "Click to change custom Levelhead." :
      ...
      reg(new GuiButton(++currentID, 1, 23, 150, 20, YELLOW + "Purchase Levelhead Credits"),
          button -> {
            try {
  ...
  private void doGeneral(DisplayConfig config, Levelhead instance, int editWidth) {
    reg(new GuiButton(++currentID, width - editWidth - 1, 29, editWidth, 20, YELLOW + "Status: "
            + (config.isEnabled() ? GREEN + "Enabled" : RED + "Disabled")),
        button -> config.setEnabled(!config.isEnabled()));
  ...
  private void doHead(DisplayConfig config, Levelhead instance, int editWidth, int mouseX,
      int mouseY) {
    ...
    reg(new GuiButton(++currentID, width / 3 - 100, height - 137, 200, 20,
        YELLOW + "Purchase Additional Above Head Layer"), button ->
        attemptPurchase("head"));
    reg(new GuiButton(++currentID, width - (editWidth << 1) - 3, 29, editWidth, 20,
            YELLOW + "Editing Layer: " +
    ...
    reg(new GuiButton(++currentID, width - (editWidth << 1) - 3, 50, editWidth, 20,
        YELLOW + "Show On Self: "
            + AQUA + (currentlyBeingEdited.getConfig().isShowSelf() ? "YES" : "NO")), button ->
        currentlyBeingEdited.getConfig()
            .setShowSelf(!currentlyBeingEdited.getConfig().isShowSelf()));

    int colorConfigStart = 93 + 2;
    reg(new GuiButton(++currentID, width - (editWidth << 1) - 2, colorConfigStart, editWidth, 20,
        YELLOW + "Header Mode: " +
    ...
    if (config.isHeaderRgb()) {
      regSlider(
          new GuiSlider(++currentID, width - (editWidth << 1) - 2, colorConfigStart + 22, editWidth,
              20, YELLOW + "Header Red: "
              + AQUA, "", 0,
      ...
      regSlider(
          new GuiSlider(++currentID, width - (editWidth << 1) - 2, colorConfigStart + 44, editWidth,
              20, YELLOW + "Header Green: "
              + AQUA, "", 0,
      ...
      regSlider(
          new GuiSlider(++currentID, width - (editWidth << 1) - 2, colorConfigStart + 66, editWidth,
              20, YELLOW + "Header Blue: "
              + AQUA, "", 0,
      ...
      reg(new GuiButton(++currentID, width - (editWidth << 1) - 2, colorConfigStart + 22, editWidth,
          20, config.getHeaderColor() +
    ...
    reg(new GuiButton(++currentID, width - editWidth - 1, colorConfigStart, editWidth, 20,
        YELLOW + "Footer Mode: " +
    ...
    if (config.isFooterRgb()) {
      regSlider(
          new GuiSlider(++currentID, width - editWidth - 1, colorConfigStart + 22, editWidth, 20,
              YELLOW + "Footer Red: " +
                  AQUA, "", 0,
      ...
      regSlider(
          new GuiSlider(++currentID, width - editWidth - 1, colorConfigStart + 44, editWidth, 20,
              YELLOW + "Footer Green: " +
                  AQUA, "", 0,
      ...
      regSlider(
          new GuiSlider(++currentID, width - editWidth - 1, colorConfigStart + 66, editWidth, 20,
              YELLOW + "Footer Blue: " +
                  AQUA, "", 0,
      ...
      reg(new GuiButton(++currentID, width - editWidth - 1, colorConfigStart + 22, editWidth, 20,
          config.getFooterColor() +
              "Rotate Footer Color"), button ->
          incrementColor(config, false));
    }

    regSlider(
        new GuiSlider(++currentID, width - (editWidth << 1) - 2, colorConfigStart + 88, editWidth,
            20, YELLOW + "Vertical Offset: " +
            AQUA, "", -50,
            100, instance.getDisplayManager().getMasterConfig().getOffset() * 100D, false, true,
            slider ->
                instance.getDisplayManager().getMasterConfig()
                    .setOffset(slider.getValue() / 100D)));

    regSlider(
        new GuiSlider(++currentID, width - editWidth - 1, colorConfigStart + 88, editWidth, 20,
            YELLOW + "Font Size: " + AQUA,
            "", 1, 20,
            instance.getDisplayManager().getMasterConfig().getFontSize() * 10D, false, true,
            slider -> {
    ...
    if (isCustom) {
      reg(new GuiButton(++currentID, width - (editWidth << 1) - 2, colorConfigStart + 88 + 22,
              (editWidth << 1) + 1, 20,
          ...
          guiButton -> {
            ...
            try {
              ...
              valueIn = new ChatComponentText(
                  "Please be logged in to your Sk1er.club for this to work. Do /levelhead dumpcache after clicking to see new colors!");
              valueIn.getChatStyle()
    ...
    GuiInventory
        .drawEntityOnScreen(posX, posY + 50, 50, posX - mouseX, posY - mouseY, mc.thePlayer);
  ...
  private void incrementColor(DisplayConfig config, boolean header) {
    int primaryId = colors
        .indexOf(removeColorChar(header ? config.getHeaderColor() : config.getFooterColor()));
  ...
  private void doChat(DisplayConfig config, Levelhead instance, int editWidth) {
    ...
    IChatComponent component = new ChatComponentTranslation(
        formattedName + WHITE + ": Levelhead rocks!");
    ...
    IChatComponent chatComponent = LevelheadChatRenderer.modifyChat(component, chatTag,
        chatDisplay != null ? chatDisplay.getConfig() : new DisplayConfig());
    ...
    if (!levelheadPurchaseStates.isChat()) {
      ...
      for (String s : text.split("\n")) {
        ...
        drawRect(tmpLeft, fakeChatTop + offset, tmpLeft + stringWidth, fakeChatTop + offset + 8,
            Integer.MIN_VALUE);
        fontRendererObj.drawString(s, tmpLeft, fakeChatTop + offset, Color.YELLOW.getRGB(), true);
        i++;
      }

      reg(new GuiButton(++currentID, width / 3 - 60, height - 140, 120, 20,
          YELLOW + "Purchase Chat Display"), button ->
          attemptPurchase("chat"));
    } else {
      reg(new GuiButton(++currentID, width - editWidth - 1, 71, editWidth, 20,
          "Rotate Bracket Color"), button ->
          incrementColor(config, true));
      reg(new GuiButton(++currentID, width - editWidth - 1, 92, editWidth, 20,
          "Rotate Value Color"), button ->
          incrementColor(config, false));
    }
  }
  ...
  private void doTab(Levelhead instance) {
    ...
    if (tab != null) {
      ...
      int totalTabWidth =
          9 + fontRendererObj.getStringWidth(formattedText) + getLevelheadWidth(playerInfo) + 15;
      int fakeTabTop = height - 200;
      int centerOn = width / 3;
      int leftStart = centerOn - totalTabWidth / 2;
      drawRect(leftStart, fakeTabTop, centerOn + totalTabWidth / 2, fakeTabTop + 8,
          Integer.MIN_VALUE);
      ...
      int l2 = 8;
      int i3 = 8;
      Gui.drawScaledCustomSizeModalRect(leftStart, fakeTabTop, 8.0F, (float) l2, 8, i3, 8, 8, 64.0F,
          64.0F);

      if (player.isWearing(EnumPlayerModelParts.HAT)) {
        int j3 = 8;
        int k3 = 8;
        Gui.drawScaledCustomSizeModalRect(leftStart, fakeTabTop, 40.0F, (float) j3, 8, k3, 8, 8,
            64.0F, 64.0F);
      }
      ...
      if (!purchaseStates.isTab()) {
        ...
        for (String s : text.split("\n")) {
          ...
          drawRect(tmpLeft, fakeTabTop + offset, tmpLeft + stringWidth, fakeTabTop + offset + 8,
              Integer.MIN_VALUE);
          fontRendererObj.drawString(s, tmpLeft, fakeTabTop + offset, Color.YELLOW.getRGB(), true);
          i++;
        }

        reg(new GuiButton(++currentID, centerOn - 60, height - 140, 120, 20,
                YELLOW + "Purchase Tab Display"),
    ...
    if (tab != null) {
      ...
      if (instance.getLevelheadPurchaseStates().isTab()) {
        ...
        if (s != null) {
          ...
          if (objective != null) {
            int score = board
                .getValueFromObjective(playerInfo.getGameProfile().getName(), objective)
                .getScorePoints();
          ...
          if (config.isFooterChroma()) {
            ...
            fontRenderer.drawString(s, x1, y,
                new Color(config.getFooterRed(), config.getFooterGreen(), config.getFooterBlue())
                    .getRGB());
          } else {
  ...
  private void attemptPurchase(String key) {
    ...
    int remainingLevelheadCredits = instance.getRawPurchases()
        .optInt("remaining_levelhead_credits");
    if (remainingLevelheadCredits < cost) {
      mc.displayGuiScreen(null);
      Hyperium.INSTANCE.getHandlers().getGeneralChatHandler().sendMessage(
          "Insufficient Levelhead credits! " + name + " costs " + cost
    ...
    if (instance.getAuth().isFailed()) {
      mc.displayGuiScreen(null);
      Hyperium.INSTANCE.getHandlers().getGeneralChatHandler().sendMessage(
          "Could not verify your identity. Please restart your client. If the issue persists, contact Sk1er.");
      return;
    }
    ...
    if (found) {
      ...
      ids.put(key.hashCode(), () -> {
        Hyperium.INSTANCE.getHandlers().getGeneralChatHandler()
            .sendMessage("Attempting to purchase: " + finalName);
        Multithreading.runAsync(() -> {
          LevelheadJsonHolder jsonHolder = new LevelheadJsonHolder(
              Sk1erMod.getInstance().rawWithAgent(
                  "https://api.sk1er.club/levelhead_purchase?access_token=" + instance.getAuth()
                      .getAccessKey() + "&request=" + key +
                      "&hash=" + instance.getAuth().getHash()));
          if (jsonHolder.optBoolean("success")) {
            instance.refreshPurchaseStates();
            Hyperium.INSTANCE.getHandlers().getGeneralChatHandler()
                .sendMessage("Successfully purchased: " + finalName);
          } else {
            Hyperium.INSTANCE.getHandlers().getGeneralChatHandler().sendMessage(
                "Failed to purchase: " + finalName + ". Cause: " + jsonHolder.optString("cause"));
          }
        });
      });
      ...
      Hyperium.INSTANCE.getHandlers().getGeneralChatHandler()
          .sendMessage("Could not find package: " + key + ". Please contact Sk1er immediately.");
    }
  }
