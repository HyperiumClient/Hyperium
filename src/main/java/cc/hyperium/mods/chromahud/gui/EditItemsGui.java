    ...
    reg(new GuiButton(nextId(), 2, 23 + 21, 100, 20, "Move Up"), button -> {
      if (modifying != null) {
        if (modifying.getOrdinal() > 0) {
          Collections
              .swap(element.getDisplayItems(), modifying.getOrdinal(), modifying.getOrdinal() - 1);
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    if (mouseButton == 0) {
      ...
      if (modifying != null) {
        ...
        for (ButtonConfig buttonConfig : ChromaHUDApi.getInstance()
            .getButtonConfigs(modifying.getType())) {
          buttonConfig.getLoad().accept(buttonConfig.getButton(), modifying);
        }

        for (StringConfig button : ChromaHUDApi.getInstance()
            .getStringConfigs(modifying.getType())) {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    for (DisplayItem displayItem : element.getDisplayItems()) {
      ...
      drawRect(xPosition, yPosition, xPosition + width, yPosition + height,
          modifying != null && modifying.getOrdinal() == displayItem.getOrdinal() ||
      ...
      fontrenderer.drawString(displayString,
          (xPosition + (width >> 1) - (fontrenderer.getStringWidth(displayString) >> 1)),
          yPosition + ((height - 8) >> 1), j,
          false);
      yPosition += 23;
    }
    ...
    if (modifying != null) {
      ...
      List<StringConfig> stringConfigs = ChromaHUDApi.getInstance()
          .getStringConfigs(modifying.getType());
        ...
        for (StringConfig config : stringConfigs) {
          ...
          for (String string : lines) {
            Minecraft.getMinecraft().fontRendererObj
                .drawString(string, xPosition, yPosition, Color.RED.getRGB());
            yPosition += 10;
          }
        }
      }
