...
public class ElementRenderer {
  ...
  public static void draw(int x, double y, List<String> list) {
    ...
    for (String string : list) {
      ...
      if (current.isHighlighted()) {
        ...
        RenderUtils
            .drawRect((int) ((x - 1) / currentScale - shift), (int) ((ty - 1) / currentScale),
                (int) ((x + 1) / currentScale)
                    + stringWidth - shift, (int) ((ty + 1) / currentScale) + 8,
      ...
      if (current.isChroma()) {
        ...
        fontRendererObj
            .drawString(string, (int) (x / currentScale - shift), (int) (ty / currentScale),
                getColor(color), current.isShadow());
      }
  ...
  private static void drawChromaString(String text, int xIn, int y) {
    ...
    for (char c : text.toCharArray()) {
      ...
      renderer
          .drawString(tmp, (float) ((double) x / currentScale), (float) ((double) y / currentScale),
              i, current.isShadow());
  ...
  public static void render(List<ItemStack> itemStacks, int x, double y, boolean showDurability,
    ...
    for (ItemStack stack : itemStacks) {
      ...
      renderItem.renderItemAndEffectIntoGUI(stack, (int) (
              (x + offset * -line + (horizontal ? (16 * line) : 0)) / ElementRenderer.getCurrentScale()
                  - (current.isRightSided() ? (showDurability ? currentScale + fontRendererObj
                  .getStringWidth(dur) : -8) : 0)),
          (int) ((y + ((horizontal ? 0 : (16 * line)) * ElementRenderer.getCurrentScale()))
              / ElementRenderer.getCurrentScale()));
      if (showDurability) {
        ...
        ElementRenderer.draw(
            (int) (x + offset * (line == 0 && dur.length() > 3 ? 1 : -line) + (horizontal ? (
                16 * line - 20) : 0) + (double) 20 * currentScale),
            y + (horizontal ? 16 : (16 * line)) + 4, dur);
      }
      line++;
    }
  ...
  public static void endDrawing(DisplayElement element) {
    GlStateManager
        .scale(1.0 / element.getScale(), 1.0 / element.getScale(), 1.0 / element.getScale());
