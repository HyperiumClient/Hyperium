...
public class GuiSidebar extends Gui {
  ...
  public enum ChromaType {
    ...
    public static ChromaType next(ChromaType current) {
      return current == ChromaType.ONE ? ChromaType.TWO
          : (current == ChromaType.TWO ? ChromaType.THREE
              : (current == ChromaType.THREE ? ChromaType.FOUR : ChromaType.ONE));
    }
  }
  ...
  public boolean contains(int mouseX, int mouseY) {
    ...
    return mouseX > minX && mouseX < maxX && mouseY > minY - fr.FONT_HEIGHT * scale
        && mouseY < maxY;
  }
  ...
  public void drawSidebar(ScoreObjective sidebar, ScaledResolution res) {
    ...
    for (Score score2 : scores) {
      ...
      if (index == scores.size()) {
        ...
        drawRect(sidebarX - 2, scoreY - fr.FONT_HEIGHT - 1, scoreX, scoreY - 1,
            getColor(true, true));
        drawRect(sidebarX - 2, scoreY - 1, scoreX, scoreY, getColor(false, true));

        drawString(s5, sidebarX + (sidebarWidth - fr.getStringWidth(s5)) / 2,
            scoreY - fr.FONT_HEIGHT,
  ...
  private int getColor(boolean darker, boolean isBackground) {
    ...
    return isBackground ? (darker ? getColorWithAlpha(rgb, Math.min(255, alpha + 10))
        : getColorWithAlpha(rgb, alpha)) : rgb;
  }
  ...
  private void drawString(String str, int x, int y, int color) {
    if (shadow) {
      fr.drawStringWithShadow(str, (float) x, (float) y, color);
    } else {
